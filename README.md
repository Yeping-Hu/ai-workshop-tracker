...
# <a href="https://aiworkshoptracker.com"><img src="./site/public/favicon.svg" alt="" width="38" height="38" align="absmiddle"></a> AI Workshop Tracker
...
- 🔎 **One unified, faceted search** across every workshop edition and 20k+ accepted-paper titles, filterable by conference, status, year, and topic, and sortable by best match, newest first, most paper matches, or name — fully static (Pagefind), so it runs entirely in the browser with no search server
...
- 🎯 **Find workshops for your paper** — paste a title and abstract and get the open calls it fits, each with its deadline, judged against every workshop's topics and the papers its earlier editions accepted; optional (it lives on the alerts satellite), and nothing you paste is stored
- 📄 **Auto-generated accepted-paper listings** for OpenReview-hosted workshops on each workshop's page
- 🗂️ **A page for each conference and each conference year** (e.g. `/conference/neurips/`, `/conference/neurips/2026/`) listing its workshops, plus the main conference's paper deadline, dates and acceptance-rate history, and one [AI conference deadlines](https://aiworkshoptracker.com/conference/) table across all of them — with schema.org structured data and an `/llms.txt` summary that make the dataset easy for search engines and AI assistants to cite
...
The one model in the pipeline is a judge, not a writer. TypeSafe's Jev answers typed yes/no and scoring questions in two scheduled jobs — which topics a newly imported workshop is about, and whether two entries are editions of one series — and, when the alerts satellite is configured, behind the optional "Find workshops for your paper" page. Every answer lands as data a person can read and overrule; the jobs fall back to their pre-Jev rules when no key is set, so a fork behaves exactly as before, and the site build never calls it. See **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**, "Typed judgments from Jev".
