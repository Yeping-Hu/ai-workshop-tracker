    ...
    if (known.has(g.id)) {
      ...
      if (raw.submission_deadline) {
        ...
        if (lastBot == null && isLegacy) {
          ...
          //
          // Declining is not the end of it. The decline writes no
          // `deadline_history` entry, so nothing downstream can infer it
          // happened — the review's `bot-long-closed` category exists precisely
          // to report it, and it reads OpenReview directly rather than our
          // history for that reason. Keep the two in step: a decline that
          // nothing reports is a workshop quietly showing as closed while
          // OpenReview says it is open.
