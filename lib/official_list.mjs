...
/**
 * Below this many characters of VISIBLE text, a response is a stub — a JS shell,
 * an error card, a challenge page — rather than a page we merely misread. Real
 * announcement pages carry tens of thousands; the committed fixtures are the
 * floor this is calibrated against.
 */
const MIN_VISIBLE_TEXT = 400;

/** Phrases that only ever appear on a wall, never on a conference blog post. */
const REFUSAL_PATTERNS = [
  /just a moment/i,
  /checking your browser/i,
  /attention required/i,
  /(?:access|request)\s+(?:denied|blocked|forbidden)/i,
  /too many requests|rate[\s-]?limit(?:ed|ing)?\b/i,
  /captcha|are you a (?:human|robot)/i,
  /security check|browser verification|ddos protection/i,
  /(?:temporarily|service) unavailable|try again (?:later|in a)/i,
  /enable (?:javascript|cookies) to continue/i,
];
...
/**
 * What did we actually receive, when it was not a workshop list?
 *
 * "0 items found" cannot tell the two causes apart, and they need opposite
 * fixes: a wrong or restructured URL is a config change, while a host refusing a
 * datacenter IP is a wait. Both arrive as HTTP 200 with a plausible byte count,
 * so the size alone decides nothing — this is what the caller carries into the
 * report instead.
 *
 * Pure, so the classification is pinned by fixtures rather than worked out live
 * at the one moment it is needed and nobody is watching.
 *
 * @returns {{ kind: 'refusal'|'stub'|'page', title: string|null, visibleChars: number, sample: string }}
 *   `refusal` — a wall; wait and re-run, change nothing.
 *   `stub`    — too little text to be the page at all.
 *   `page`    — a real page we failed to extract from; the bug is the extractor.
 */
export function describeResponse(html) {
  const raw = String(html ?? '');
  const title = text(raw.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] ?? '') || null;
  // Visible text only. A JS shell is almost entirely <script>, and counting that
  // would make an empty page look substantial — the exact mistake byte counts
  // already make.
  const visible = text(raw.replace(/<(script|style|noscript|template|svg)\b[^>]*>[\s\S]*?<\/\1>/gi, ''));
  const kind = REFUSAL_PATTERNS.some((re) => re.test(`${title ?? ''} ${visible}`))
    ? 'refusal'
    : visible.length < MIN_VISIBLE_TEXT
      ? 'stub'
      : 'page';
  return { kind, title, visibleChars: visible.length, sample: visible.slice(0, 180) };
}
