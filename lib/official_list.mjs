...
export function statedWorkshopCount(html) {
  ...
  for (const m of flat.matchAll(/(\d+)\s+(?:accepted\s+)?workshops?\b/gi)) {
    const n = Number(m[1]);
    // "…the NeurIPS 2026 accepted workshops" matches the same shape as "…102
    // accepted workshops". A year is never a plausible count, so skip it rather
    // than report a cross-check that is guaranteed to disagree.
