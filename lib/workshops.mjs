...
/**
 * Which edition a conference's homepage card is about, and what is known of it.
 *
 * The year is the one a reader can act on: the year of the conference's open
 * calls when it has any, else the soonest edition still to end (one ending
 * today is still on), else the latest tracked. The dates come from
 * data/editions.yml when that year has a row; a year with open calls and no
 * row is reported with no edition rather than the previous year's dates —
 * CoRL 2026 had nineteen open calls before its dates were recorded, and a card
 * saying "CoRL 2025 · 19 open calls" would have been wrong in the way that
 * matters. Pure over its inputs; pinned by scripts/conference_strip_test.mjs.
 */
export function conferenceCard(conferenceId, { workshops, editions, nowMs = Date.now() }) {
  const endMs = (e) => parseDateUtcMs(String(e?.end ?? ''));
  const rows = (editions ?? []).filter((e) => e?.conference === conferenceId && endMs(e) != null);
  const openYears = (workshops ?? [])
    .filter((w) => w.conference === conferenceId && w.status === 'upcoming' && w.deadlineUtcMs != null)
    .map((w) => w.year)
    .filter((y) => Number.isFinite(y));
  const todayMs = Math.floor(nowMs / 86_400_000) * 86_400_000;
  const ahead = rows.filter((e) => endMs(e) >= todayMs).sort((a, b) => endMs(a) - endMs(b));
  const latest = [...rows].sort((a, b) => endMs(b) - endMs(a))[0] ?? null;
  const year = openYears.length ? Math.max(...openYears) : ((ahead[0] ?? latest)?.year ?? null);
  const edition = year != null ? (rows.find((e) => e.year === year) ?? null) : null;
  return {
    year,
    openCalls: openYears.length,
    edition,
    upcoming: edition ? endMs(edition) >= todayMs : openYears.length > 0,
  };
}
