...
export function deriveDeadlineChange(history, nowMs = Date.now(), parseDeadline, formatWallClock = (v) => v) {
  ...
  const from = parseDeadline(prev.value);
  const to = parseDeadline(last.value);
  ...
  return {
    ...
    fromWallClock: formatWallClock(prev.value),
    daysAgo,
 ...
 */
export function recordDeadlineObservation(raw, value, today) {
  ...
  if (hist.length === 0 && norm(raw.submission_deadline) != null) {
    ...
    hist.push({ value: norm(raw.submission_deadline), recorded: asOf || raw.added || today });
  }

  const tail = hist.length ? hist[hist.length - 1] : null;
  if (tail && norm(tail.value) === next) return false;

  hist.push({ value: next, recorded: today });
...
export function resolveWorkshop({ slug, file, raw }, nowMs = Date.now(), confMonths = {}, editionEnds = {}) {
  ...
  return {
    ...
    deadlineChange: deriveDeadlineChange(
      ...
      nowMs,
      (v) => resolveDeadlineUtcMs(v, effectiveTimezone || raw.timezone || 'AoE'),
      (v) => formatDeadlineWallClock(v, effectiveTimezone || raw.timezone || 'AoE'),
    ),
    deadlineHistoryView: Array.isArray(raw.deadline_history)
      ? raw.deadline_history
          .map((h) => ({
            ...
            wallClock: h.value
              ? formatDeadlineWallClock(h.value, effectiveTimezone || raw.timezone || 'AoE')
              : null,
          }))
          .reverse()
      : [],
