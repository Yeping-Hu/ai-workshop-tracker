 ...
 *  Exported so lib/extensions.mjs applies the same floor to the net move. */
...
export function resolveWorkshop({ slug, file, raw }, nowMs = Date.now(), confMonths = {}, editionEnds = {}) {
  ...
  return {
    ...
    // The raw log, oldest first, for corpus-wide derivations (lib/extensions.mjs
    // nets each entry's first value against its last). The view below is
    // rendered and newest-first, so it cannot serve that.
    deadlineHistory: Array.isArray(raw.deadline_history) ? raw.deadline_history : [],
