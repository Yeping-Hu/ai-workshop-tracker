...
export function readWorkshopFile(filePath) {
  ...
  return { slug: slugOfFile(filePath), file: path.relative(REPO_ROOT, filePath), raw };
}

/** The file a slug lives in, or null. The slug IS the basename, so this is a
 *  stat rather than a walk-and-parse of the corpus (three scripts did the
 *  latter to find one file); a `.yaml` spelling is honoured like everywhere else. */
export function workshopFilePath(slug) {
  if (!/^[a-z0-9][a-z0-9-]*$/.test(String(slug ?? ''))) return null;
  for (const ext of ['.yml', '.yaml']) {
    const p = path.join(WORKSHOPS_DIR, `${slug}${ext}`);
    if (fs.existsSync(p)) return p;
  }
  return null;
}

/** The slug of a workshop file path, however the extension is spelled. */
export const slugOfFile = (filePath) => path.basename(filePath).replace(/\.ya?ml$/, '');
...
export function lastDataChange() {
  ...
  // Raw entries: only `added` and the history stamps are needed, and a full
  // loadWorkshops() here (locations, relations, every paper cache) ran the
  // whole corpus derivation a second time per build, from the config graph.
  const out = new Map(listWorkshopFiles().map((f) => {
    const { slug, raw } = readWorkshopFile(f);
    return [slug, inYaml(raw ?? {})];
  }));
 ...
 */
export function venueStem(venueId) {
...
/**
 * Pairs Jev judged to be one series, and the decisions people recorded over
 * them — the input to Tier 5 of computeRelations(). `data/series_links.yml` is
 * written only by scripts/series_audit.mjs (lib/series_links.mjs holds the
 * candidate rules and the question). It is READ here so the build stays
 * offline: the audit judges on Sunday, the build links on every deploy, and a
 * checkout with no file — every fork — links exactly as Tiers 1–4 alone do.
 */
export const SERIES_LINKS_FILE = path.join(REPO_ROOT, 'data', 'series_links.yml');

export function loadSeriesLinks() {
  if (!fs.existsSync(SERIES_LINKS_FILE)) return { model: null, pairs: [], decisions: [] };
  const raw = yaml.load(fs.readFileSync(SERIES_LINKS_FILE, 'utf8')) ?? {};
  return {
...
/**
 * The probability of "same series" at or above which a judged pair links with
 * no person in the loop; from REVIEW_MIN up to it, a person is asked instead.
 *
 * Read off the real pairs the "What still does not link" note names, judged
 * by jev-1.13.0 on 2026-09-18: the four true series (DPFM→Data_Problems→
 * DATA-FM, FM4LS and SPIGM across conferences) came back 0.90–0.99; the AIMS
 * collision — "AI Measurement Science" against "AI for Mechanism Design" —
 * 0.00; and the H2R collision, two "humans to robots" workshops at different
 * venues, 0.13 with the mass split between "different" and "unclear". 0.9
 * links the first group, keeps both collisions nowhere near a link, and hands
 * the one genuinely ambiguous pair to a person, which is where it belongs.
 * Precision over recall, as everywhere in this file: an unlinked series is the
 * safe failure. Not a Choice threshold and not a noul — a Score's per-level
 * probability, and TypeSafe's own note says not to carry one across kinds.
 */
export const LINK_MIN = 0.9;
export const REVIEW_MIN = 0.4;

/**
 * The identity a judgment was made on, as a short hash: name, acronym,
 * website, registered stem, conference, year. A recorded pair applies only
 * while both entries still hash as they did when judged — a renamed or
 * re-homed entry is a new question, not a stale answer. Deadlines, notes and
 * status are deliberately outside it: the daily jobs move those constantly
 * and none of them is evidence of identity.
 */
export function identityHash(e) {
  const parts = [e.name ?? '', e.acronym ?? '', e.website ?? '', venueStem(e.openreview_venue_id) ?? '', e.conference ?? '', String(e.year ?? '')];
  return crypto.createHash('sha1').update(JSON.stringify(parts)).digest('hex').slice(0, 12);
}

/** One pair is one record however it was found: slugs in sorted order. */
export function pairKey(a, b) {
  return a < b ? `${a}|${b}` : `${b}|${a}`;
}

/** The hash a pair record carries — both identities, in canonical order. */
export function pairHash(ea, eb) {
  const [x, y] = ea.slug < eb.slug ? [ea, eb] : [eb, ea];
  return crypto.createHash('sha1').update(identityHash(x) + identityHash(y)).digest('hex').slice(0, 12);
}

/**
 * Which recorded pairs link today. A `same` decision always does. A judged
 * pair does when its probability clears LINK_MIN, its hash still matches both
 * entries, and no `different` decision overrides it — a decision outranks a
 * probability in either direction. A slug that has left the dataset links
 * nothing; the audit prunes it on its next run.
 */
export function linkedPairs(seriesLinks, entries) {
  if (!seriesLinks) return [];
  const by = new Map(entries.map((e) => [e.slug, e]));
  const decided = new Map((seriesLinks.decisions ?? []).map((d) => [pairKey(d.a, d.b), d.verdict]));
  const out = [];
  const seen = new Set();
  const push = (a, b) => {
    const k = pairKey(a, b);
    if (seen.has(k) || !by.has(a) || !by.has(b)) return;
    seen.add(k);
    out.push(a < b ? [a, b] : [b, a]);
  };
  for (const d of seriesLinks.decisions ?? []) if (d.verdict === 'same') push(d.a, d.b);
  for (const p of seriesLinks.pairs ?? []) {
    if (decided.get(pairKey(p.a, p.b)) === 'different') continue;
    if (!(typeof p.same === 'number' && p.same >= LINK_MIN)) continue;
    const ea = by.get(p.a);
    const eb = by.get(p.b);
    if (!ea || !eb || pairHash(ea, eb) !== p.hash) continue;
    push(p.a, p.b);
  }
  return out;
}

export function computeRelations(entries, { seriesLinks = null } = {}) {
  ...
  // Tier 5: pairs Jev judged to be one series, or a person recorded as such.
  //
  // Tiers 1–4 identify a series by an ADDRESS it keeps — a site, a registered
  // stem — and the two shapes none of them reaches were measured and left, in
  // "What still does not link" (ARCHITECTURE.md): a series that moves
  // conference AND domain at once (FM4LS, SPIGM), and one that renames its
  // stem (DPFM / Data_Problems / DATA-FM, three identical names on three
  // sites). Both are visible only to a reader of the names, and that note's
  // own condition for widening was "a pair-by-pair audit". scripts/
  // series_audit.mjs is that audit — Jev reads each code-generated candidate
  // pair once a week — and data/series_links.yml is its record.
  //
  // Nothing is asked here. A link applies only while both entries still hash
  // as they did when judged (linkedPairs), so a renamed entry is re-asked
  // rather than trusted, and a recorded decision outranks any probability in
  // either direction. With no file this tier is a no-op, which is what every
  // fork gets. Union-find makes the link transitive, exactly as a shared site
  // is: FM4LS's NeurIPS edition joining either ICML one joins all three.
  const bySlug = new Map(entries.map((e, i) => [e.slug, i]));
  for (const [a, b] of linkedPairs(seriesLinks, entries)) unite(bySlug.get(a), bySlug.get(b));
...
export function loadWorkshops(nowMs = Date.now()) {
  ...
  // plus the pairs the weekly audit judged (Tier 5), read from the file it
  // writes; absent, every fork, and the derivation is Tiers 1–4 alone.
  const relations = computeRelations(all, { seriesLinks: loadSeriesLinks() });
  for (const w of all) {
 ...
 *  Memoised for the life of the process: a site build asks for every cache
 *  from five places (loadWorkshops, the homepage's expected counts, the
 *  no-PDF endpoint, each workshop page), which was ~45 MB of JSON parsed for
 *  8.7 MB of files. Nothing writes a cache and then reads it back in one
 *  process — the monthly refresh writes files directly — so a per-process
 *  memo cannot go stale. */
const paperCacheMemo = new Map();
export function loadPaperCache(slug) {
  if (paperCacheMemo.has(slug)) return paperCacheMemo.get(slug);
  ...
  paperCacheMemo.set(slug, parsed);
  return parsed;
}

/** The rows of data/proposal_calls.yml as written — for the validator and the
 *  daily sync, which rewrites the file. An absent file is an empty list. */
export function loadProposalCallRows() {
...
/** The newest recorded call for workshop proposals of a conference — the
 *  latest year, then the latest deadline within it — or null. */
export function latestProposalCall(calls, conferenceId) {
  return (
    (calls ?? [])
      .filter((c) => c.conference === conferenceId)
      .sort((a, b) => (Number(b.year) - Number(a.year)) || ((b.deadlineUtcMs ?? 0) - (a.deadlineUtcMs ?? 0)))[0] ?? null
  );
}
