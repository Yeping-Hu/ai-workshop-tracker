...
export function resolveWorkshop({ slug, file, raw }, nowMs = Date.now(), confMonths = {}, editionEnds = {}) {
  ...
  return {
    ...
    // abstract registration. The headline/countdown above deliberately stay on
    // the paper deadline so the displayed date never jumps forward and read as
    // an extension; this is surfaced beside it and, once passed, stays visible
    // marked closed rather than disappearing (which is what would make a later
    // paper deadline look like an extension). Stored as UTC wall-clock.
