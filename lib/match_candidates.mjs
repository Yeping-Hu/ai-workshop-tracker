 ...
 *
 * Which dozen: the first met walking the series newest edition first, in the
 * cache's own order — alphabetical by title (scripts/fetch_openreview.mjs). A
 * latest edition with twelve papers or more is therefore the whole sample,
 * from the head of its alphabet. Deterministic, and the same for every paper.
 */
