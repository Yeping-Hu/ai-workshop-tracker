...
/** The far edge of a believable deadline: two years (with leap days) ahead. */
export const TWO_YEARS_MS = 2 * 366 * DAY_MS;

/**
 * Is a deadline read from OpenReview believable for this edition? A parseable
 * instant, no more than two years out, in a year within one of the edition's.
 * Every write path applies exactly this before adopting a fetched value —
 * one garbled read must skip an entry, not fail validation for the run — and
 * four scripts carried their own copy, two of which had already drifted.
 */
export function plausibleDeadline(fetchedMs, fetchedYear, editionYear, nowMs = Date.now()) {
  return (
    fetchedMs != null &&
    Number.isFinite(fetchedMs) &&
    fetchedMs - nowMs <= TWO_YEARS_MS &&
    Number.isFinite(fetchedYear) &&
    Math.abs(fetchedYear - editionYear) <= 1
  );
}
...
/**
 * The wall clock "YYYY-MM-DD HH:MM" of a UTC instant in a zone (AoE, UTC or an
 * IANA name): the inverse of zonedToUtcMs. The editions sync uses it to write
 * a second deadline in the zone a row already has, so the two deadlines of one
 * call never carry different zones.
 */
export function utcMsToWallClock(ms, zone = 'UTC') {
  ...
  if (zone === 'AoE') return fromUtc(ms - 12 * 3_600_000);
  if (zone === 'UTC') return fromUtc(ms);
  if (!isValidTimezone(zone)) return null;
  return fromUtc(ms + tzOffsetMinutes(ms, zone) * 60_000);
}
...
/**
 * An edition's dates as one phrase — "Dec 6–13, 2026", "Sep 27 – Oct 1, 2026",
 * "Dec 30, 2026 – Jan 2, 2027" — or whichever single date is known, or null.
 * One definition for the conference-year page and the homepage cards, so the
 * same edition cannot read two ways.
 */
export function formatDateSpan(start, end) {
  const sMs = start != null ? parseDateUtcMs(String(start)) : null;
  const eMs = end != null ? parseDateUtcMs(String(end)) : null;
  const s = sMs != null ? formatDateYmd(String(start)) : null;
  const e = eMs != null ? formatDateYmd(String(end)) : null;
  if (s && e) {
    if (sMs === eMs) return s;
    const [sm, sd, sy] = s.split(/[ ,]+/);
    const [em, ed, ey] = e.split(/[ ,]+/);
    if (sy !== ey) return `${s} – ${e}`;
