 ...
 * hosts it under its own OpenReview namespace. So `offList` is a bucket for a
 * human to read, never an instruction. Nothing here writes anything.
 *
 * Where the line is. Off the list AND outside the conference's OpenReview
 * namespace is a different fact: the organisers run the event on their own.
 * UniReps 2026 and ML4PS 2026 were both imported from NeurIPS.cc during the
 * proposal phase, were absent from the 102 accepted, and within days had moved
 * to their own namespaces with websites that no longer claimed NeurIPS. Such an
 * entry is not the conference's workshop, however co-located, and is recorded
 * `not_running`; an acknowledgement recorded before the move was a verdict on a
 * different fact, so it is reported again as `independent`. `hostedByConference`
 * is that one predicate, and the corpus bears it out: when the rule was written
 * (2026-09-04) 935 of 936 OpenReview-linked entries sat in their conference's
 * namespace, and the one that did not was ML4PS.
 */
import { websiteKey, siteRoot, nameTokens, namesAgree } from './workshops.mjs';

/**
 * Is this venue id under the conference's own OpenReview namespace?
 *
 * `venueNamespace` is the conference-year listing discovery crawls
 * (`NeurIPS.cc/2026/Workshop`). An id under it was created by the conference's
 * proposal process and is hosted by the conference whether or not the list
 * names it — affinity events and competitions live there. An id anywhere else
 * (`ML4PS/2026/Workshop`, `UniReps.org/2026/Workshop`) belongs to the organisers
 * alone. Null when there is nothing to compare — no namespace known, or no venue
 * id — because "unknown" must not read as either answer.
 */
export function hostedByConference(venueId, venueNamespace) {
  const id = String(venueId ?? '').trim();
  const ns = String(venueNamespace ?? '').trim().replace(/\/+$/, '');
  if (!id || !ns) return null;
 ...
 * @param venueNamespace  the conference-year's OpenReview namespace, for hostedByConference()
 */
export function matchOfficialList(entries, listed, { listUrl = null, conferenceWebsite = null, venueNamespace = null } = {}) {
  ...
  // An acknowledgement says "running, and merely not a workshop in this list's
  // sense" — recorded against the venue the entry had at the time. An entry that
  // has since moved out of the conference's namespace has changed the fact the
  // verdict rested on: the organisers have left, and the expected verdict is now
  // `not_running`. Only acknowledged entries are looked at, since an
  // unacknowledged one is already in `offList`, where its row says the same.
  const independent = acked.filter((e) => hostedByConference(e.openreview_venue_id, venueNamespace) === false);
