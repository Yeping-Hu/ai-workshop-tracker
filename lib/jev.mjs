 ...
 *
 * Pure: no filesystem, and the network and the clock are injectable so
 * `scripts/jev_client_test.mjs` runs offline.
 */
...
const usage = { requests: 0, input_tokens: 0, failures: 0 };
...
export async function askJev(state, questions, {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      usage.failures++;
      warnOnce(`unreachable after ${retries + 1} attempts (${err?.message ?? err})`);
      return null;
    }
    if (res.ok) {
    ...
    warnOnce(`HTTP ${res.status}${text ? ` (${text.slice(0, 200)})` : ''}`);
    return null;
  }
}
