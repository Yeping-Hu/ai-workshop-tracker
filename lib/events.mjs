...
/** "2026-09-01" -> "1 Sep 2026": the day-first form every email row and the
 *  page's date column already use. '' for anything unparseable. */
export function fmtDay(day) {
  const t = day ? Date.parse(`${String(day).slice(0, 10)}T00:00:00Z`) : NaN;
...
/**
 * The week as one phrase — "1 Sep 2026 – 7 Sep 2026" — from the feed's own
 * `since` and `until`.
 *
 * Built here, once, because the digest and /changes/ each had their own: the
 * mail printed "31 Aug 2026 – 7 Sep 2026" (both ends, from the run clock) while
 * the page printed "since 1 Sept 2026" (one end, through the browser's locale
 * tables), and a reader following the link from the one to the other saw two
 * different weeks. Both ends, always, and the same words on both surfaces.
 */
export function windowLabel(since, until) {
  const s = fmtDay(since);
  const u = fmtDay(until);
  if (s && u) return s === u ? s : `${s} – ${u}`;
  return s || u || '';
}
