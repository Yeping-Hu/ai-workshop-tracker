 ...
 */
/**
 * The topic id out of one checkbox's text. The forms show `<id> — <Label>`
 * (scripts/gen_topic_options.mjs) so a contributor can tell what `graphs` or
 * `affinity` covers; the id is everything before the dash. A bare id — every
 * issue opened before 2026-09-19, and `other` still — passes through unchanged.
 * An en dash or a spaced hyphen is accepted as well, because the text has been
 * through GitHub's renderer and possibly an email client by the time it is read;
 * a hyphen inside an id has no spaces around it, so ids are safe.
 */
const topicIdOf = (text) => text.split(/\s+[—–-]\s+/)[0].trim().toLowerCase();
