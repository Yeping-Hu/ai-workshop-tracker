 ...
 * nothing cleared the bar — or null when no judgment could be had at all.
 * `entry` needs name, acronym, conference and year. Either way the caller falls
 * back to the keyword table, but the two are different facts and every caller
 * needs the difference. An import records it: "no answer" marks the entry as
 * still owed a judgment (topicsForImport in discover_openreview.mjs), so the
 * fallback heals the next time Jev can be asked, while "nothing fits" is a
 * considered answer and is final. The re-judging sweeps act on it: "no answer"
 * must leave a judged entry exactly as it is, or an outage would overwrite the
 * corpus with keyword guesses.
 */
