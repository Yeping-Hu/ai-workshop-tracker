...
| `TYPESAFE_API_KEY` | pipeline jobs that ask Jev (`lib/jev.mjs`) | TypeSafe API key for the typed judgments — topic tags when an entry is imported, the weekly series-identity audit. A repo Action **secret**; the same variable locally for the sweeps; and a Worker secret (`wrangler secret put TYPESAFE_API_KEY` from `alerts/worker/`) for the paper matcher's `/match`. **Unset ⇒ every job falls back to its pre-Jev rule** (the keyword table tags topics; the audit records nothing). The site build never reads it — see ARCHITECTURE.md, "Typed judgments from Jev" | off |
