...
| `discover.yml` | weekly | Discovers new workshops/venues/deadlines from OpenReview, backfills a `website`/deadline/tracks that organizers published after the venue was imported, and syncs extensions (later-only); each new venue's topics are judged by Jev, with the keyword table standing in when no `TYPESAFE_API_KEY` is set → commits to `main`. Opens the `data-health` issue "Jev did not answer" when requests to Jev failed (closed by the next run in which every request succeeded) |
