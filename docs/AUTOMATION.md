...
|---|---|---|
| `validate.yml` | PRs & pushes touching data, `schema/`, `scripts/`, `lib/` or a workflow | Schema + sanity checks and every standalone suite; comments fixes on the PR. Workflows are in the trigger because a dozen suites read them and one runs the JavaScript inside two of them |
...
| `discover.yml` | weekly | Discovers new workshops/venues/deadlines from OpenReview, backfills a `website`/deadline/tracks that organizers published after the venue was imported, and syncs extensions (later-only); each new venue's topics are judged by Jev, with the keyword table standing in when Jev cannot be asked — and the run ends by re-judging any entry still marked as tagged that way (`retag_topics.mjs --pending`), so an outage costs its imports a week, not their topics → commits to `main`. Opens the `data-health` issue "Jev did not answer" when requests to Jev failed — quoting the error and listing, with links, the workshops still waiting for a judgment (`scripts/waiting_topics.mjs`) — closed by the next run in which every request succeeded |
...
| `series-audit.yml` | weekly, manual | `scripts/series_audit.mjs` — asks Jev whether each candidate pair of entries (a shared OpenReview stem across conferences, or agreeing names across years in one conference; nothing already linked) is one workshop series, records every answer in `data/series_links.yml` → commits to `main`, and keeps one `data-health` issue ("workshop series to confirm") of the pairs between 0.4 and 0.9. The build links what cleared 0.9 (ARCHITECTURE.md, "Related entries", signal 5). Dispatching it with two slugs and `same`/`different` records a person's verdict, which outranks the model. Without a `TYPESAFE_API_KEY` it asks nothing; when requests to Jev failed it opens the same "Jev did not answer" issue discovery does, waiting-workshops list included, since whichever job runs last rewrites the body |
...
- **A new workshop's topics are judged, not keyword-matched.** Discovery asks
  ...
  revisited (ARCHITECTURE.md, "Typed judgments from Jev"). When Jev could not
  be asked at all the note says so in a second sentence, and the next weekly
  run that can ask re-judges those entries and takes the sentence off — the
  job heals its own bad Sundays.
...
- **A new edition or a new track** joins its series with nothing to wire up. The
  ...
  renames itself past recognition while keeping its venue id, still links. A
  series that moves conference and domain at once, or renames its registered
  stem, is what none of those see; the Sunday audit asks Jev about exactly
  those pairs (`series-audit.yml`), and a link it records follows on the next
  build.
