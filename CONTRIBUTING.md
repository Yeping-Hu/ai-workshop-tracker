...
| `topics` | ✅ | 1–5 ids from `data/topics.yml` (the add/edit issue forms offer these as a checkbox list that shows each id with its label — `graphs — Graphs & geometry` — so there are no typos or unknown ids, and no guessing what an id covers) |
...
| `review_ack` |  | An OpenReview value you reviewed and chose **not** to adopt (`name`, `acronym`, `website` and/or `submission_deadline`), which stops the weekly review reporting it. Stores the rejected value rather than muting the entry: if OpenReview later changes to something different, you are told again. Also takes `official_list`: the accepted-workshop list URL you checked when confirming an entry is legitimately absent from it — an affinity event or competition the conference still hosts under its own OpenReview namespace. An event whose venue lies outside that namespace is not the conference's, however co-located, and gets `not_running` instead. An acknowledgement is re-reported if the entry's venue later leaves the namespace, or if `data/editions.yml` is pointed at a newer list. |
| `added` |  | `YYYY-MM-DD` — feeds the "new workshops" RSS |
| `not_running` |  | This edition is **not taking place** — usually because the conference's official accepted-workshop list does not include it (OpenReview creates a venue group during the *proposal* phase, so a rejected proposal keeps a live group with a ticking deadline). Needs `reason` (`not_on_official_list` / `withdrawn` / `cancelled`) and `recorded`; record a `source` too. The entry keeps its page — deleting the file does not work, the next crawl re-creates it — but stops being an Open call, stops feeding alerts, and is frozen against every deadline sync. Set it with `scripts/mark_not_running.mjs`, not by hand. A later verdict supersedes the earlier one: marking an entry that carries `review_ack.official_list` drops that acknowledgement, and acknowledging a marked entry unmarks it, each saying so. |
