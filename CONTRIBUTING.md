...
| `review_ack` |  | An OpenReview value you reviewed and chose **not** to adopt (`name`, `acronym`, `website` and/or `submission_deadline`), which stops the weekly review reporting it. Stores the rejected value rather than muting the entry: if OpenReview later changes to something different, you are told again. Also takes `official_list`: the accepted-workshop list URL you checked when confirming an entry is legitimately absent from it — an affinity event, a competition, a co-located workshop. Point `data/editions.yml` at a newer list and every such acknowledgement is re-reported. |
